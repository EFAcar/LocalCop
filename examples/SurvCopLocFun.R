...
nll_obj <- SurvCopLocFun(
  ...
  family = family,
  x = rep(5, n),
  x0 = 0, # centered covariate x - x0 == 0
...
system.time({
  ll1 <- sapply(rhovec, stucop_lik) # TMB version
})


# likelihood function based on R implementation
...
system.time({
  ll2 <- sapply(rhovec, stucop_lik2) # R version
})
